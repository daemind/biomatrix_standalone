...
class UnionOperator(LogicOperator):
    ...
    def apply(self, state: State) -> State:
        ...
        # Merge all points with Mass-Prioritized Occlusion (Painter's Algo)
        valid_results = [r for r in results if not r.is_empty]
        if not valid_results:
             return State(np.empty((0, state.n_dims)))
             
        # Sort by Causality (Low -> High) then Mass (Big -> Small)
        # 1. Causality Ascending (0.0 First, 1.0 Last -> 1.0 Overwrites)
        # 2. Mass Descending (Big First, Small Last -> Small Overwrites)
        valid_results.sort(key=lambda s: (s.causality_score, -s.n_points))
        
        # Stack points
        all_points = np.vstack([r.points for r in valid_results])
        
        # ALGEBRAIC: Strict N-Dimensional Agnostic Union
        # We do NOT assume distinct "spatial" vs "value" dimensions here.
        # "Overwrite" logic is only valid if we project onto a subspace.
        # Here, we perform pure Set Union (Manifold Merge).
        # Any "Collision" (e.g. same x,y, diff color) results in co-existing points in P.
        # This preserves all information. Projection/Rendering handles occlusions later.
        
        # Unique points (Set Union)
        # Note: distinct values at same coords are distinct points in N-dims.
