...
AGENT.md Compliant:
- NO for/while loops
- NO if/else chains in logic
- Algebraic operations only (dot product, matrix ops, broadcasting)
- N-dimensional agnostic

This is the main entry point. Specialized derivation functions are in:
- affine.py: Affine transforms (matched, centered, scaled, permutation)
- permutation.py: Value permutations, rank transforms
- lifting.py: Lifting, tiling, envelope, replication
- union.py: Union detection, component matching, deletion
"""
...
# Import derive_causality from algebra.py (via get_algebra?)
# Actually derive_causality was defined in derive/algebra.py.
# But derive/algebra.py is imported as a module.
# Let's import it from .algebra if possible, or use _get_algebra().derive_causality logic.
# However, derive_causality relies on derive_transformation (recursive).
# In algebra.py, derive_causality was added.
# Let's import it carefully.
...
# --- CONFIGURATION ---
# --- CONFIGURATION ---
# Controls the scientific rigor of the solver.
# Tier 1: Trivial (Identity, Subset)
# Tier 2: Algebraic Core (Affine, Lifting, Separation) - TARGET STATE
# Tier 3: Compositional (Union, Partition)
MAX_TIER = 4 
...
def derive_transformation(s_in: State, s_out: State, depth: int = 0, priors: List[Callable] = None) -> Optional[Operator]:
    """
    Master Law Resolver: S_in -> S_out.
    
    SCIENTIFIC PIPELINE (DECLARATIVE):
    Iterates through the SOLVER_PIPELINE based on MAX_TIER.
    This structure ensures "Loud and Clear" visibility of the execution order.
    """
    # Recursion Guard
    if depth > 2:
        return None
    ...
    for tier, name, strategy_func in SOLVER_PIPELINE:
        ...
        try:
            ...
            # We catch exceptions to prevent one heuristic from crashing the solver,
            # but we explicitly log them if in debug mode.
            # For now, silent continue is standard for "Try/Fail" heuristics.
            pass
...
def derive_all_transformations(s_in: State, s_out: State, depth: int = 0) -> List[Operator]:
    ...
    Useful for Champion-Challenger to find the one that generalizes.
    """
    if depth > 2:
        return []
...
def derive_subset(s_in: State, s_out: State, tol: float = 0.1) -> Optional[Operator]:
    """
    Derive Subset Relation (S_out < S_in).
    
    PURE N-DIM AGNOSTIC (AGENT.md):
    Find the dimension that DISCRIMINATES S_out from S_in.
    """
    if s_out.n_points == 0:
        return None
    ...
    # Check Dimension Compatibility (Fix for ce8d95cc)
    if s_out.n_dims != s_in.n_dims:
        return None
        
    # 1. Check pointwise inclusion (In-Place Subset)
