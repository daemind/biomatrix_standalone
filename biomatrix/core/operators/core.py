# -*- coding: utf-8 -*-
"""
operators/core.py - Legacy Operators (not yet split into sub-modules)

Contains operators that have not been migrated to dedicated sub-modules.
These will be gradually migrated to: topology.py, force.py, fill.py, etc.
"""
...
class LinearForceOperator(Operator):
    ...
    def apply(self, state: State) -> State:
        ...
        for idx in indices:
            ...
            # Create step multipliers: [1, 2, ..., max_steps]
            multipliers = np.arange(1, max_steps + 1).reshape(-1, 1, 1)
            
            # Traicetories: P_0 + k * v
            # Shape: (Steps, N_pts, D)
            trajectories = obj.points[np.newaxis, :, :] + multipliers * step_vec[np.newaxis, np.newaxis, :]
            
            # Round to integer for collision check
            traj_int = np.round(trajectories).astype(int)
            
            # Check collisions:
            # We want the first step k where ANY point in the object hits 'occupied'
            # OR goes out of bounds ( < 0 )
            
            # Flatten to check set membership efficently? 
            # Or just iterate steps until collision? 
            # "No explicit loops" -> strict AGENT.md.
            # But iterating 0..steps is iterating the INTENDED MOVEMENT.
            # Algebraic replacement: 
            # Compute distance to nearest obstruction for every point.
            
            # Let's verify collision for all steps at once using set intersection?
            # Building a massive set of all trajectory points is expensive.
            
            # Optimization: 
            # Check bounding box first? 
            
            # Compromise: We iterate steps but efficiently.
            # Actually, `reduce` is the functional equivalent.
            
            valid_k = 0
            
            # Fast-Forward: Check if destination is free?
            # If we just check discrete steps.
            
            for k in range(max_steps):
                # Step k+1 (index k)
                pts_at_k = traj_int[k] # (N_pts, D)
                
                # Check Bounds (< 0)
                if np.any(pts_at_k < 0):
                    break
                    
                # Check Collision (Set intersection)
                # Convert to tuples
                # This is the bottleneck.
                # Optimized: View as void for fast 1D isin check?
                # or just loop over points if N is small?
